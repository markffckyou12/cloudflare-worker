...
describe('provisionSlackWorkflow', () => {
  ...
  test('full happy path: resolve ref_no -> matter+officers from Supabase -> createChannel -> write back -> updateMatterCard', async () => {
    ...
    mockFetch((url, options) => {
      ...
      if (u === 'https://slack.com/api/conversations.setPurpose') {
        expect(parsed.channel).toBe('C999');
        seen.push('setPurpose');
        return { ok: true };
      }
      if (u === 'https://slack.com/api/conversations.setTopic') {
        expect(parsed.channel).toBe('C999');
        expect(parsed.topic).toContain('Active'); // status
        expect(parsed.topic).toContain('U111'); // PIC
        seen.push('setTopic');
        return { ok: true };
      }
    ...
    expect(seen).toEqual(['getMatterIdByRefNo', 'getMatterForSlackProvisioning', 'getMatterOfficersForSlack', 'createChannel', 'updateMatter', 'setPurpose', 'setTopic', 'updateMatterCard']);
  });
...
describe('updateMatter officer sync (direct Slack API)', () => {
  test('invites only those Slack itself confirms are missing, removes only tracked officers no longer wanted, then refreshes topic+card', async () => {
    const seen = [];
    ...
    mockFetch((url, options) => {
      ...
      if (u.includes('/rest/v1/matter_officers') && method === 'DELETE') {
      ...
      // Previous officer list (A + B) -- used only to decide who's safe
      // to remove (must be a currently-tracked officer, not just any
      // channel member).
      ...
      // New officer id list (B, C) -> Slack ids.
      ...
      if (u.includes('/rest/v1/matters?id=eq.99&select=ref_no')) {
        seen.push('getMatterForSlackProvisioning');
        return { __body: [{ ref_no: 'TP/001', client_name: 'Acme', progress_pct: 50, status: 'Active', slack_channel_id: 'C123', slack_message_ts: '111.222', project_types: { name: 'Litigation' } }] };
      }
      if (u.includes('/rest/v1/audit_log')) return { __body: null };

      // Ground truth: Slack says A and B are currently in the channel
      // (admin, from the bot itself, is deliberately NOT in this list --
      // proving the diff never tries to touch someone the officer system
      // doesn't track).
      if (u === 'https://slack.com/api/conversations.members') {
        seen.push('getChannelMembers');
        return { ok: true, members: ['U_A', 'U_B'] };
      }
      if (u === 'https://slack.com/api/conversations.invite') {
        const body = JSON.parse(options.body);
        expect(body.channel).toBe('C123');
        expect(body.users).toBe('U_C'); // only the one Slack confirms is actually missing
      ...
      if (u === 'https://slack.com/api/conversations.kick') {
        const body = JSON.parse(options.body);
        expect(body.channel).toBe('C123');
        expect(body.user).toBe('U_A'); // only the dropped, formerly-tracked officer
        seen.push('slackKick');
        return { ok: true };
      }
      if (u === 'https://slack.com/api/conversations.setTopic') {
      ...
      const parsed = options?.body ? JSON.parse(options.body) : null;
      if (parsed?.action === 'updateMatterCard') {
        seen.push('updateMatterCard');
        return { success: true };
      }
    ...
    expect(body.success).toBe(true);
    expect(seen).toContain('getChannelMembers');
    expect(seen).toContain('slackInvite');
    expect(seen).toContain('slackKick');
    expect(seen).toContain('setTopic');
    expect(seen).toContain('updateMatterCard');
    // B (unchanged, and never removed from Slack) never triggers invite or kick -- only the diff does.
  });

  test('does not re-invite someone Slack already shows as a member, even if our own officer-list snapshot is stale', async () => {
    // This is the actual bug report this rewrite fixes: an officer
    // (e.g. the default admin) already sitting in the channel getting a
    // redundant invite attempt. Simulate exactly that -- desired list
    // includes someone Slack says is already there.
    const seen = [];
    const token = await makeStaffToken('auth-user-1');
    ...
    mockFetch((url, options) => {
      ...
      if (u.includes('/rest/v1/matter_officers?matter_id=eq.')) return { __body: [{ staff: { name: 'Admin', initial: 'ADMIN', slack_member_id: 'U_ADMIN' } }] };
      if (u.includes('/rest/v1/staff?id=in.')) return { __body: [{ id: 1, slack_member_id: 'U_ADMIN' }] };
      if (u.includes('/rest/v1/matters?id=eq.7&select=ref_no')) {
        return { __body: [{ ref_no: 'TP/003', client_name: 'Gamma', progress_pct: 0, status: 'Draft', slack_channel_id: 'C777', slack_message_ts: null, project_types: { name: 'Tax' } }] };
      }
      if (u.includes('/rest/v1/audit_log')) return { __body: null };
      if (u === 'https://slack.com/api/conversations.members') return { ok: true, members: ['U_ADMIN'] }; // already there
      if (u === 'https://slack.com/api/conversations.invite') { seen.push('slackInvite'); return { ok: true }; }
      if (u === 'https://slack.com/api/conversations.kick') { seen.push('slackKick'); return { ok: true }; }
      if (u === 'https://slack.com/api/conversations.setTopic') { seen.push('setTopic'); return { ok: true }; }
      return { __body: null };
    });
    ...
    const req = new Request('https://worker.example/', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ action: 'updateMatter', matterId: 7, officerStaffIds: [1] }) // admin stays the only officer, unchanged
    });
    const res = await worker.fetch(req, ENV, {});
    const body = await res.json();

    expect(body.success).toBe(true);
    expect(seen).not.toContain('slackInvite'); // Slack already shows them present -- never called
    expect(seen).not.toContain('slackKick');
    expect(seen).toContain('setTopic'); // card/topic still refresh even with no membership change
  });
